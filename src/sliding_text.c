...
static void update_proc_sliding_row(Layer *layer, GContext *ctx) {
  ...
  if (row_data && row_data->text) {
    ...
    draw_text(ctx, row_data->text, row_data->font);  
  }
}
...
static void handle_init() {
  ...
  init_sliding_row(data, &data->rows[0], GRect(0, 27, width, 41), 6, BOLD_TYPE);
  layer_add_child(window_layer, data->rows[0].layer);

  init_sliding_row(data, &data->rows[1], GRect(0, 63, width, 41), 3, LIGHT_TYPE);
  layer_add_child(window_layer, data->rows[1].layer);

  init_sliding_row(data, &data->rows[2], GRect(0, 99, width, 41), 0, LIGHT_TYPE);
